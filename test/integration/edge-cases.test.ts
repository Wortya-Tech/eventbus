...
describe("edge-cases - duplicate subscribe", () => {
    ...
    it("should not throw when subscribing with existing key", () => {
        const h = async () => {};
...
describe("edge-cases - publish fail", () => {
    ...
    it("publish() should return false when not connected", async () => {
...
describe("edge-cases - close error", () => {
    let connection: ChannelModel;
    let svc: EventBusService;
    ...
    it("close() should handle double close gracefully", async () => {
        await svc.close();
        await svc.close();
        assert.ok(true);
    });
